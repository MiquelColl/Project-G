...
const shoppingCart = () => {
  ...
  addToShoppingCartButtons.forEach((addToCartButton) => {
    ...
    function addToCartClicked(event) {
      ...
      datosToLocalStorage(localStorageContainer, datosLocalStorage);
    ...
    function addGameToShoppingCart(gameTitle, gamePrice, gameImage) {
      ...
      for (let i = 0; i < elementTitle.length; i++) {
        if (elementTitle[i].innerText === gameTitle) {
          let elementQuantity = elementTitle[
            i
  ...
  `;
      ...
      shoppingCartRow
        ...
        .addEventListener("change", quantityChanger);
    ...
    function updateShoppingCartTotal() {
      ...
      console.log(
        "🚀 ~ file: shop.js:116 ~ updateShoppingCartTotal ~ shoppingCartItems:",
        shoppingCartItems
      );
...
// localStorageContainer.push(JSON.parse(localStorage.getItem("carrito")))

function datosToLocalStorage(StorageContainer, datos) {
  let duplicado = 0;
  console.log(StorageContainer);
  if (StorageContainer.length == 0) {
    StorageContainer.push(datos);
  } else {
    StorageContainer.forEach((e) => {
      // console.log('dato 1' ,e.gameTitle);
      // console.log('dato 2' ,datos.gameTitle);
      if (e.gameTitle == datos.gameTitle) {
        e.gameQuantity++;
    ...
    localStorage.setItem("carrito", JSON.stringify(localStorageContainer));
    // StorageContainer.push(JSON.parse(localStorage.getItem("carrito")))
  }
}
...
function init() {
  renderGames();
  console.log("Dentro de local storage hay: ", localStorage.getItem("carrito"));
  // localStorage.clear()
}
